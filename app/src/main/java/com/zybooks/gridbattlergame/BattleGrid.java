...
public class BattleGrid {
    ...
    public void performAttack(int index) {
        ...
        switch (AbilityType) {
            ...
            case MAGIC:
                int[] radius = getSpecialRadius(currentTarget, 2, "Enemy");
                for (int i = 0; i < radius.length; i++) {
                    if (radius[i] != -1) {
                        setContent(radius[i], "enemy");
                    }
                }
            case EXPLOSIVE:
                int[] exAdjacent = getSpecialAdjacent(currentTarget,  "Enemy");
                for (int i = 0; i < exAdjacent.length; i++) {
                    if (exAdjacent[i] != -1) {
                        setContent(exAdjacent[i], "enemy");
                        }
                }
        }
    }
